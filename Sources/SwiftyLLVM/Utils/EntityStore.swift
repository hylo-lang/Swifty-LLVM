/// Manages access to a set of native handles using an identity based approach.
public struct EntityStore<Entity: LLVMEntity>: ~Copyable {

  /// The ever-growing list of entity handles, identified by their index in the list via `Entity.Identity`.
  ///
  ...
  /// Inserts an entity handle into the store and starts managing it, returning its new identity.
  ///
  ...
  internal mutating func insert(_ handle: Entity.Handle) -> Entity.Identity {
    let id = Entity.Identity(uncheckedFrom: .init(handles.count))
  ...
  public subscript(_ id: Entity.Identity) -> Entity {
    mutating _read {
      ...
      yield Entity(temporarilyWrapping: handle)
    }
    _modify {
      let handle = unsafeExtract(id)
      var entity = Entity(temporarilyWrapping: handle)
      defer { unsafeRestore(id, handle) }
      yield &entity
    }
  }
  ...
  public func handle(for id: Entity.Identity) -> Entity.Handle? {
