...
func read<T, U>(_ value: T, _ action: (T) throws -> U) rethrows -> U {
  try action(value)
}

/// Returns the application of `action` on a mutable projection of `value`.
func modify<T, U>(_ value: inout T, _ action: (inout T) throws -> U) rethrows -> U {
  try action(&value)
}
...
func update<T>(_ value: inout T, with transform: (T) throws -> T) rethrows {
